...
class Feedback 
{
  constructor(context)
  {
    ...
    this.filter.setFreq(440);
    this.filter.setQ(0.25);
    ...
    this.feedbackVal = 0;
    this.feedbackGain = 25.0;

    this.feedbackVal2 = 0;
    this.feedbackGain2 = 25.0;

    this.reverbFeedbackGain = 8.0;
    ...
    this.processor.onaudioprocess = function(audioProcessingEvent) {
      ...
      for (let i = 0; i < outputBuffer.length; i++) 
      {
          ...
          let nextInput = 0.25 * Math.atan(inputData[i]*self.reverbFeedbackGain + self.feedbackVal*self.feedbackGain + self.feedbackVal2*self.feedbackGain2);
...
feedbackGenerator.connect(reverb);
...
$('#slider').range2DSlider({
  ...
  {
    let x = val[0] * 0.1;
    var newDelayTime2 = Math.floor((x * x) * 300) + 325;
    feedbackGenerator.setDelayTime2 (newDelayTime2)

    let y = val[1] * 0.1;
    var newDelayTime = Math.floor((y * y) * 50) + 125;
