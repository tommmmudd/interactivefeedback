...
class GutterOsc 
{
  ...
  setGain(newGain) {this.gain = newGain;}

  setRate(newRate) {this.omega = newRate;}

  setMod(newMod) {this.gamma = newMod;}
...
for (let i=0; i<8; i++)
{
  ...
  gainSliders[i].addEventListener('input', function() 
  {
    gutterOscs[i].setGain((this.value * this.value) * 5.0);
  }, "false");
  gainSliders[i].value = 0.5

  dampSliders[i] = document.querySelector(".damp-slider"+(i+1));
  dampSliders[i].addEventListener('input', function() 
  {
    gutterOscs[i].setDamping((this.value * this.value * this.value) + 0.0001);
  }, "false");
  dampSliders[i].value = 0.7400823; // ^4 = 0.3
  ...
  rateSliders[i] = document.querySelector(".rate-slider"+(i+1));
  rateSliders[i].addEventListener('input', function() 
  {
    gutterOscs[i].setRate((this.value * this.value * this.value * this.value) * 40.0);
  }, "false");
  rateSliders[i].value = 0.282

  resSliders[i] = document.querySelector(".res-slider"+(i+1));
  resSliders[i].addEventListener('input', function() 
  {
  ...
  randButtons[i] = document.querySelector(".minor"+(i+1));
  randButtons[i].onmousedown = function() 
  {
      let note = minorScale[i] + 24;
      gutterOscs[i].minorFilters(note);
...
masterGainSlider.addEventListener('input', function() 
{
  ...
  {
    gutterOscs[i].setGain(this.value * this.value * 5.0);
    gainSliders[i].value = this.value;
  }
}, "false");
masterGainSlider.value = 0.32

const masterDampSlider = document.querySelector(".master-damp-slider");
masterDampSlider.addEventListener('input', function() 
{
  for (let i=0; i<8; i++)
  {
    gutterOscs[i].setDamping(this.value * this.value * this.value + 0.0001);
    dampSliders[i].value = this.value;
  }
}, "false");
masterDampSlider.value = 0.7

const masterModSlider = document.querySelector(".master-mod-slider");
masterModSlider.addEventListener('input', function() 
{
  for (let i=0; i<8; i++)
  {
    gutterOscs[i].setMod((this.value) * 1.0);
...
masterModSlider.value = 0.2

const masterRateSlider = document.querySelector(".master-rate-slider");
masterRateSlider.addEventListener('input', function() 
{
  for (let i=0; i<8; i++)
  {
    gutterOscs[i].setRate(this.value * this.value * this.value * this.value * 40.0);
    rateSliders[i].value = this.value;
  }
}, "false");
masterRateSlider.value = 0.282

const masterResSlider = document.querySelector(".master-res-slider");
masterResSlider.addEventListener('input', function() 
{
  for (let i=0; i<8; i++)
  {
    gutterOscs[i].setQ(this.value * this.value * 200.0 + 0.001);
